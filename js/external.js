...
function addHighlight(element) {
    if (element) {
        ...
        else if (element.tagName === "u") element.style.textDecoration = "underline " + backgroundColor + " solid";
    }
}
    ...
    var addColor = element => {
        ...
        if (element.querySelector("input").getAttribute("type") === "radio") {
            ...
            for (let index = 0; index < inputs.length; index++) {
                ...
                if (inputs[index].getAttribute("name") !== name) {
                    continue
                }
...
function showSpecialQuestion(article, section) {
    ...
    if (section.children[0].children[1].innerText.length < 3) {
        ...
        insertArea.forEach(elem => {
            if (elem.getAttribute("data-answer") == "A") {
                elem.scrollIntoView();
            }
...
function filterTag(e) {
    ...
    if (!input) {
        input = createNode(["input", {
            class: "w3-bar-item w3-hide w3-right",
            id: "filter",
            autofocus: true
        }, ""], parent);
    }
...
function updateNav() {
            ...
            for (let index = 1; index <= parseInt(element.split(":")[1]); index++) {
                ...
                if (mobileFlag) {
                    let a = createNode(["a", {
                        ...
                        href: href
                    }, section + " " + index], dropdownContent);
                } else {
                    type = element.split(":")[0].replace("ing", "").replace("Writ", "Write");
                    let a = createNode(["a", {
                        class: "w3-padding-small w3-button " + color,
                        href: href
                    }, type + " " + index], div);
                }
            }
    ...
    if (setFlag) {
        ...
        categoryBtn = createNode(["a", {
            class: "w3-btn w3-left w3-section w3-large " + color,
            href: href
        }, "See Same Category Questions"], div);
        if (testFlag) testBtn = createNode(["button", {
            class: "w3-btn w3-right w3-section w3-large " + color,
            id: "test"
        }, "Test"], div);
        if (mobileFlag) {
        ...
        function filterSet(element) {
            ...
            document.querySelectorAll("#setsDiv a").forEach(elem => {
                if (element.split(":")[1].includes(elem.href.split("/").splice(-1)[0])) {
                    ...
                    createNode(["a", {
                        class: "w3-left w3-button w3-padding-small my-margin-small " + color,
                        href: elem.href
                    }, innerText], setDiv);
                }
            })
        ...
        for (let i = 0; i < 4; i++) {
            button = createNode(["button", {
                class: "w3-bar-item w3-button w3-col l2"
            }, sections[i].split(":")[0]], div);
            if (mobileFlag) {
                button.className = button.className.replace("w3-col l2", "w3-padding-small");
            }
            button.onclick = () => {
                ...
                categoryString.split("&")[i].split(";").forEach(element => {
                    ...
                    button = createNode(["button", {
                        class: "tag w3-btn w3-padding-small my-margin-small highlight"
                    }, tag], categroyDiv);
        ...
        let search = createNode(["button", {
            class: "w3-bar-item w3-button w3-right"
        }, "Search"], div);
        search.onclick = e => filterTag(e);

        if (mobileFlag) {
            search.classList.toggle("w3-padding-small")
        }
        categroyDiv = createNode(["div", {
...
function updateNotes() {
    ...
    function createEntry(entries) {
        ...
        entries.forEach(element => {
            let div = createNode(["div", {
                class: "w3-card w3-padding w3-left my-margin w3-white my-entry"
            }, ""], entriesDiv);
            ...
            let tagDiv = createNode(["div", {
                class: "w3-white w3-section"
            }, ""], div);
    ...
    if (document.querySelector("#tagDiv")) {
        let search = createNode(["button", {
            class: "w3-bar-item w3-button w3-right w3-padding-small"
        }, "Search"], document.querySelector("#tagDiv").children[0]);
        search.onclick = e => filterTag(e);
    }
    ...
    if (!(/\/notes\.html|\/blog\.html/).exec(uri)) {
        ...
        let div = createNode(["div", {
            class: "w3-card w3-white w3-padding w3-section"
        }, ""], main, true);
...
function startTest() {
    ...
    function playAudio(link, onEnd) {
        ...
        if (promise !== undefined) {
            promise.catch(error => {
                ...
                if (!button) {
                    button = createNode(["button", {
                        class: "w3-btn w3-block w3-section w3-hide " + color,
                        id: "playAudio"
    ...
    function navigateQuestion(thisElem) {
        if (uri.includes("listening")) {
            id = thisElem.previousElementSibling.id;
        }
    ...
    function checkAnswer(id) {
        ...
        if (answer.length < 2 || questions[id - 1].getAttribute("data-choice-type") == "select") {
            ...
            if (myAnswer[id - 1] !== answer) {
                myAnswer[id - 1] += "->" + answer
            };
    ...
    function showModal() {
        ...
        if (!buttonBar) {
            ...
            exitBtn = createNode(["button", {
                class: "w3-btn w3-margin w3-left " + color
            }, "Save and Exit"], buttonBar);
            cancelBtn = createNode(["button", {
                class: "w3-btn w3-margin w3-right " + color
            }, "Cancel"], buttonBar);
        }
        if (uri.includes("speaking")) {
    ...
    function showQuestion(index) {
        ...
        if (!reviewBtn) {
            reviewBtn = createNode(["button", {
                class: "w3-btn w3-block w3-half " + color,
                id: "review"
            }, "Review Questions"], testDiv);
        }
        reviewBtn.onclick = () => reviewQuestions(id.split("n")[1]);
        div.querySelectorAll("button").forEach(elem => {
            if (mobileFlag) {
                elem.classList.toggle("w3-padding-small");
            }
    ...
    function addTextarea(note) {
        ...
        wordCountDiv = createNode(["div", {
            class: "w3-half w3-padding w3-section"
        }, ""], testDiv);
    ...
    if (greFlag) {
        ...
        if ((/issue|argument/).exec(uri)) {
            ...
            time = createNode(["p", {
                id: "time",
                class: "w3-xxlarge w3-center my-margin-small"
            }, ""], testDiv);
        ...
        time = createNode(["p", {
            id: "time",
            class: "w3-jumbo w3-center w3-half"
        }, ""], testDiv);
...
function updateUI() {
    ...
    function showQuestion(article) {
        ...
        for (let i = 0; i < questions.length; i++) {
            let button = createNode(["button", {
                class: "w3-bar-item w3-button " + color
            }, i + 1], pageBar);
            if (uri.includes("reading") && mobileFlag) {
            ...
            button.onclick = e => {
                ...
                // Verbal reasoning
                if (greFlag) {
                    toggleHighlight(questionDiv.querySelector(`span[data-question="${e.target.textContent}"]`));
                    ...
                    btn.onclick = e => {
                        ...
                        if (/\d+/.exec(answer.childNodes[0].innerText)) {
    ...
    if (greFlag) { // Update Verbal Reasoning UI
        ...
        //document.querySelectorAll(".choices").forEach(element => element.classList.add("w3-hide"));
...
function initialize() {
    ...
    document.querySelectorAll(".highlight, h1, h2, h3, h4, h5, h6, b, u, em").forEach(element => addHighlight(element));
    ...
    if ((/practice|sample/).exec(uri) && !greFlag) {
        updateUI(questions);
        if (testFlag) createNode(["button", {
            class: "w3-btn w3-right " + color,
            id: "test"
        }, "Test"], main.children[0]);
    }

    if (greFlag) {
        updateUI();
        if (testFlag) createNode(["button", {
            class: "w3-btn w3-right w3-margin " + color,
            id: "test"
        }, "Test"], main, true);
    }
    ...
    if (uri.includes("vocabulary")) {
        createWordSets();
        createNode(["button", {
            class: "w3-bar w3-btn w3-right w3-padding w3-section " + color
        }, "Recite"], main.children[0]).onclick = createWordTest;
    }
...
function setListStyle() {
    ...
    document.querySelectorAll("li").forEach(li => {
        ...
        li.innerHTML = "<span style=\"color:black;font-weight:normal\">" + li.innerHTML + "</span>"
    });
...
function createWordSets() {
    ...
    function showWordModal(word) {
        ...
        for (let i = 0; i < details.length; i++) {
            ...
            let button = createNode(["button", {
                class: "w3-btn my-margin-small w3-padding-small " + color,
                id: details[i]
            }, details[i]], div);
            button.onclick = (e) => {
                ...
                if (e.target.textContent == "examples") {
                    ...
                    detailDiv.querySelectorAll("b").forEach(node => {
                        addHighlight(node);
                    });
                    ...
                    detailDiv.querySelectorAll(".Syn").forEach(element => {
                        ...
                        element.querySelectorAll(".w3-hide").forEach(node => {
                            if (node.nextSibling) node.nextSibling.textContent = ""
                        });
                    });
                    detailDiv.innerHTML = detailDiv.innerHTML.replace(/<br>/g, "")
                    detailDiv.querySelectorAll("b.w3-hide").forEach(node => {
                        if (node.innerText == "Quotations") node.parentNode.classList.add("w3-hide")
                    });
        ...
        createNode(["button", {
            class: "w3-btn w3-bar w3-padding-small w3-section " + color
        }, "close"], div).onclick = () => {
            modal.style.display = "none";
    ...
    sets.forEach(set => {
        ...
        var button = createNode(["button", {
            class: "w3-btn my-margin-small " + color
        }, set.name], main);
        ...
        button.onclick = () => {
            ...
            words.forEach(word => {
                ...
                var buttonDiv = createNode(["div", {
                    class: "w3-card my-margin w3-padding-small w3-center w3-border w3-large"
                }, word.word], wordDiv);
...
function createWordTest() {
    ...
    function showReviewModal(set) {
        ...
        forgottenWords.forEach(word => {
            ...
            var wordDiv = createNode(["div", {
                class: "w3-left w3-col l3"
            }, ""], div);
            var buttonDiv = createNode(["div", {
                class: "my-margin-small w3-padding-small w3-center"
            }, ""], wordDiv);
            var detailDiv = createNode(["div", {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                button.onclick = (e) => {
                    ...
                    // button hide
                    createNode(["button", {
                        class: "w3-btn w3-bar w3-padding-small my-margin-top-small " + color
                    }, "hide"], detailDiv).onclick = () => {
                        detailDiv.innerHTML = "";
        ...
        modal.style.display = "block";
        createNode(["button", {
            class: "w3-btn w3-padding w3-margin-top w3-bar " + color
    ...
    function showQuestion(set) {
        ...
        createNode(["button", {
            class: "w3-btn w3-padding w3-section w3-bar " + color
            ...
            for (let i = 0; i < labels.length; i++) {
                ...
                if (event.target.textContent == "Check" && !answer.includes(i)) {
                    ...
                    let audio = createNode(["audio", {
                        src: words[wordNumber].sound
                    }, ""], label);
        ...
        createNode(["button", {
            class: "w3-btn w3-padding w3-bar " + color
