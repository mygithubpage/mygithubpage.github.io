...
function Invoke-InternetExplorer {
    Param($Uri, $Condition)
    $ie = (New-Object -COM "Shell.Application").Windows().ForEach{ if($_.Name -like "*Internet Explorer*") {$_}}
...
function ConvertTo-UnicodeNumber {
    ...
    $prefix = if($content -cmatch " Dim \w+ As \w+") { "&H" } else { "\u"}
    $pattern = "\p{IsCJKUnifiedIdeographs}|\p{IsHalfwidthandFullwidthForms}|\p{IsEnclosedAlphanumerics}"
    ($content | Select-String $pattern -AllMatches).Matches.Value.ForEach{
        $byteString = "$([Text.Encoding]::Unicode.GetBytes($_))"
        $characters = $_
        for ($i = 0; $i -lt $byteString.Split(" ").Count; $i+=2) {
            $byte1 = $byteString.Split(" ")[$i]
            $byte2 = $byteString.Split(" ")[$i+1]
            $content = $content -replace $characters , ($prefix + (ConvertTo-HexNumber $byte2) + (ConvertTo-HexNumber $byte1))
        }
    }
    $content
}
...
function ConvertTo-Code {
    ...
    ($content | Select-String $pattern -AllMatches).Matches.Value.ForEach{
        $byteString = "$([Text.Encoding]::Unicode.GetBytes($_))"
        $string = ""
        if($encoding -match "utf8") {
            $byteString.Split(" ").ForEach{ $string += ( "%" + (ConvertTo-HexNumber $_) ) }
        }
        else {
            for ($i = 0; $i -lt $byteString.Split(" ").Count; $i+=2) {
                $byte1 = $byteString.Split(" ")[$i]
                $byte2 = $byteString.Split(" ")[$i+1]
                $string += ($prefix + (ConvertTo-HexNumber $byte2) + (ConvertTo-HexNumber $byte1))
            }
        }
        $content = $content -replace $_, $string
    }
    $content
}
