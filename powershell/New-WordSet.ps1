...
function Get-Etymology ($word) {
    ...
    $html = Invoke-WebRequest $uri
    $document = $html.ParsedHtml.body
    $etymology = $document.getElementsByTagName("p").ForEach{ if ($_.previousSibling.innerText -like "*Etymology*") { $_.innerHTML } }
...
function Get-WordFamily ($word) {
    ...
    if ($innerHtml) {
        ...
        foreach ($match in ($innerHtml | Select-String $regex -AllMatches -CaseSensitive).Matches) {
            $members += New-Object PSObject @{ Word = $match.Groups["word"].Value; Parent = $match.Groups["parent"].Value}
        }
    }
    else {
        ...
        $members += New-Object PSObject @{ Word = $words[0]; Parent = ""}
        for ($i = 1; $i -lt $words.Count; $i++) {
            ...
            while (!$parent)
            $members += New-Object PSObject @{ Word = $words[$i]; Parent = $parent}
        }
    }
    Get-Children ""
...
$set = New-Object PSObject @{name = $name}
...
$details = ($sets[0].words[0] | Get-Member -MemberType).Name
