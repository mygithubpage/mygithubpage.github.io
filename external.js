...
function initialize() {
    ...
    if(!document.title) document.title = titleCase(html.split(".")[0].replace("-", " "))
    ...
    if((/practice|sample/).exec(uri)) { 
        updateUI(); 
        if((/verbal|quantitative/).exec(uri)) testBtn = createNode(["button", {class:"w3-btn w3-right w3-margin " + color, id:"test"}, "Test"], main, true).createGreTest;
    ...
    if(uri.includes("vocabulary")) {
        createWordSets();
        createNode(["button", {class:"w3-bar w3-btn w3-right " + color}, "Recite"], main.children[0]).onclick = createWordTest;
    }
    ...
    vocabularySets.forEach(element => {
        ...
        button.onclick = () => {
            ...
            words.forEach(element => {
                ...
                var termDiv = createNode(["div",{class:"highlight w3-xlarge"},word], buttonDiv);
...
function createWordTest() {
    ...
    function showQuestion(randomSet) {
        ...
        options.forEach(element => {
            ...
            let audio = createNode(["audio",{src:vocabularySets[randomSet][1][element.split(",")[1]][1][3].split(",")[1]},""], testDiv);
